...
# ========== 超參數 ========== 
input_dim = 6  # 每個tick有6個特徵
seq_length = 64  # 64個tick
hidden_dim = 1024  # GRU隱藏層維度
batch_size = 32
learning_rate = 1e-4
num_epochs = 100
patience = 10
...
class GRUClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_weights = torch.softmax(self.attention_layer(gru_out).squeeze(-1), dim=1)
...
# ========== EarlyStopping ========== 
class EarlyStopping:
    def __init__(self, patience=10, save_path="best_gru_model.pt"):
...
# ========== 訓練 ========== 
model = GRUClassifier(input_dim=input_dim, hidden_dim=hidden_dim, dropout_rate=dropout_rate).to(device)
...
print("最佳模型已儲存為 'best_gru_model.pt'")
